    ...
    public class Program
    {
        ...
        public RecommendCourseView RecommendedCourseForStudent(List<Enrollment> studentEnrollments, Schedule fallSchedule, Schedule winterSchedule, List<Schedule> summerSchedules)
        {
            ...
            foreach (CourseSequence courseSequence in CourseSequences.OrderBy(n=> n.Year))
            {
                ...
                if (noEnrollment)
                {
                    if (((fallSchedule == null && recommendCourseView.RecommendFallCourseSequence.Count() < 5) 
                        || (fallSchedule != null && fallSchedule.Enrollments.Count < 5 && recommendCourseView.RecommendFallCourseSequence.Count() < 5 - fallSchedule.Enrollments.Count))
                        && (courseSequence.Course == null || courseSequence.Course.MissingPrequisite(enrollmentCopy, recommendCourseView.RecommendFallCourseSequence, db.Semesters.Where(n => n.Season == Season.Fall).FirstOrDefault()).Count == 0))
                            recommendCourseView.RecommendFallCourseSequence.Add(courseSequence);
                    else if (((winterSchedule == null && recommendCourseView.RecommendWinterCourseSequence.Count() < 5) 
                        || (winterSchedule != null && winterSchedule.Enrollments.Count < 5 && recommendCourseView.RecommendWinterCourseSequence.Count() < 5 - winterSchedule.Enrollments.Count))
                        && (courseSequence.Course == null || courseSequence.Course.MissingPrequisite(enrollmentCopy, recommendCourseView.RecommendWinterCourseSequence, recommendCourseView.RecommendFallCourseSequence, db.Semesters.Where(n => n.Season == Season.Winter).FirstOrDefault()).Count == 0))
